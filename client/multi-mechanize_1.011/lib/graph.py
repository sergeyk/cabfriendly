...
def resp_graph_raw(nested_resp_list, image_name, dir='./'):
    ...
    ax.set_xlabel('Elapsed Time In Test (secs)', size='large')
    ax.set_ylabel('Response Time (secs)' , size='large')
...
def resp_graph(avg_resptime_points_dict, percentile_80_resptime_points_dict, percentile_90_resptime_points_dict, image_name, dir='./'):
    fig = figure(figsize=(8, 4.2))  # image dimensions  
    ax = fig.add_subplot(111)
    ax.set_xlabel('Elapsed Time In Test (secs)', size='large')
    ax.set_ylabel('Response Time (secs)' , size='large')
    ax.grid(True, color='#666666')
    ...
    ax.plot(x_seq, y_seq, 
        ...
        markeredgecolor='green', markerfacecolor='yellow', markersize=3.0)
    ...
    ax.plot(x_seq, y_seq, 
        ...
        markeredgecolor='orange', markerfacecolor='yellow', markersize=3.0)
    ...
    ax.plot(x_seq, y_seq, 
        ...
        markeredgecolor='purple', markerfacecolor='yellow', markersize=3.0)
...
def tp_graph(throughputs_dict, image_name, dir='./'):
    fig = figure(figsize=(8, 4.4))  # image dimensions  
    ax = fig.add_subplot(111)
    ax.set_xlabel('Elapsed Time In Test (secs)', size='large')
    ax.set_ylabel('Transactions Per Second (count)' , size='large')
    ax.grid(True, color='#666666')
    xticks(size='large')
    yticks(size='large')
    ...
    ax.plot(x_seq, y_seq, 
        ...
        markeredgecolor='red', markerfacecolor='yellow', markersize=3.0)
    ax.plot([0.0,], [0.0,], linewidth=0.0, markersize=0.0)
    savefig(dir + image_name) 
